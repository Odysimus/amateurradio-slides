...
\subtitle{Original Presentation ``Amateur Radio in a Weekend'' \\  by Dr. John Stanley N8GFO}
\AtBeginSection[]
{
	\begin{frame}<*>
		\begin{multicols}{2}
...
\begin{frame}
\frametitle{Ham not HAM}
You will often hear of amateur radio referred to as ham radio. Ham is not an acronym or initialism so only capitalize as you would any other normal noun. The term comes from the early days when some felt amateurs were ``ham-fisted'' and not as good as professional radio operators. Think of the term ``ham actor''. Well joke's on them because we liked it so much we embraced it, and ``ham'' is much faster to say than ``Amateur Radio Operator''.
\end{frame}
...
We have a higher priority than these unlicensed users. Unlicensed  users must not cause us interference and must accept interference from our operations.
\end{frame}
...
\begin{frame}
\frametitle{NTS vs ICS 213}
The NTS form is often too short for important messages. The ICS 213 is designed for use in offices with forms in triplicate, and doesn't have the checks that NTS has. Largely the ICS 213 is preferred by emergency communication groups as they are what their served agencies use. Most ICS 213 messages are sent using a digital system. Sending pages of documents over the air by voice or CW is tedious and prone to errors.
\end{frame}
...
\item[QRZ] I'm calling/Who's Calling?
...
\begin{frame}
\frametitle{APRS}
The Automatic Packet Reporting System APRS (pronounced A-pers) encodes position data in standard packet formats. Other information such as weather, messages, even icons. The system has ``Igates'' or internet gateways that pass information to internet servers. \hyperlink{www.findu.com}{findu.com} and \hyperlink{www.aprs.fi}{aprs.fi}
\end{frame}
...
Always be sure to use the proper fuse, they are rated for voltage, current, and slow/fast burn. Also think about why a fuse burned when replacing it. Many a electronics tinkerer has burned through a pack a fuses by not fixing a problem. 
\end{frame}

...
In the world of DC circuits measureing things like power and current are simple. In the world of AC (which radio is a part) it is more difficult, for one thing in DC all electrons slowly move in one direction, in AC they just kind of wiggle back and forth. The other thing is that if one uses normal caluclus over a sine wave the positive and negative parts cancle eachother out mathmatically. We do have a few tricks up our sleeve.
\end{frame}

\begin{frame}
\frametitle{PEP, RMS, TPO}
For the AC line votalge from say a wall outlet we simplify the things by using a system called Root Mean Squared or RMS. With this we take the square root of the average of the square of the voltage, which allows you to use $P=IV$. 120V is an RMS value actuall peak to peak voltage is much larger
...
\being{frame}
\frametitle{Common Curisies}
Amateur radio is unique in the level of freedom, and lack of oversight we are given. We are able to get away with this becuase by and large we play nice with each other and with other users of radio spectrum. This has its own drawbacks, for one many "rules" are unwritten or just gentlemen's agreements. The last bit of information isn't part of the test but should help you get on the air faster.
\end{frame}

\begin{frame}
\frametitle{Footprint}
Remember that when using phone modes that you are take up a piece of that band and that at any given time only one station can use any given piece. Anything you can do to reduce your footprint in congested bands will allow more people to use them. Turning down your power to comply with the mininmal power requirement can go a long way. Using directional antennas can reduce the amount of power needed even more.
\end{frame}
